...
%       turbine is known
%   LOCATION: 3-element vector containing coordinates of where to evaluate 
%       a value of wind speed
%   POWER (output): Nx1 vector containing power of each turbine
%   SPEED (output): The scalar wind-speed at point LOCATION
%
...
for jj=1:nturbs
	...
	if(xt(jj)<xm)
        ...
        [ywake,zwake]=wakecentre(xm,afactor,yaw_rads(jj),xt(jj),yt(jj),zt(jj),diameter(jj));
        dwake(3)=wakeexpand_q(xm,3,diameter(jj),xt(jj));
        
        %If point is within wake, get velocity at point
        if(norm([ym-ywake,zm-zwake])<=0.5*dwake(3))
            ii=ii+1;
            
            %Wake centre line
            [ywake,zwake]=wakecentre(xm,afactor,yaw_rads(jj),xt(jj),yt(jj),zt(jj),diameter(jj));
            dwake(1)=wakeexpand_q(xm,1,diameter(jj),xt(jj));
            dwake(2)=wakeexpand_q(xm,2,diameter(jj),xt(jj));
            dwake(3)=wakeexpand_q(xm,3,diameter(jj),xt(jj));

            %Wake decay
            vwake(1)=wakedecay_q(xm,1,diameter(jj),yaw_rads(jj),xt(jj));
            vwake(2)=wakedecay_q(xm,2,diameter(jj),yaw_rads(jj),xt(jj));
            vwake(3)=wakedecay_q(xm,3,diameter(jj),yaw_rads(jj),xt(jj));

            %Wake zones
            rad=sqrt((ym-ywake)^2+(zm-zwake)^2);
            if(rad <= dwake(1)/2)
                wakedecay=vwake(1);
...
if((max(loc1,loc2)~=3) && (min(loc1,loc2)~=1))
    disp('ERROR: location must be a 3-element vector');
    disp(['Currently size is ',num2str(loc1),'x',num2str(loc2)]);
    err=true;
end
